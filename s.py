...
# The server should return with their number e.g 1,2,3 
...
def startUP(server_num):
    try: 
        ...
        os.system(f"start python {sys.argv[0]} {server_num}")  # Start new server
...
@method
def online():
    current_port = 5000 + int(serverNumber)
    for friend_port in myFriends:
        try:
            response = requests.post(f'http://localhost:{friend_port}/online', json={'port': current_port})
            if response.status_code == 200:
                print(f"Notified server on port {friend_port} that server {current_port} is online.")
        except Exception as e:
